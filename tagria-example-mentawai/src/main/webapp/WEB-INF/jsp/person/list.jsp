	...
	label="List of persons" export="true" paginate="false">
		...
		<html:gridColumn>
			<html:buttonGroup spaced="true">
				<html:button url="/Person.edit.mtw?id=${person.id}" icon="pencil"/>
				<html:button state="danger" icon="trash" id="${ person.id }" />
				<html:confirm label="Are you sure about delete this person ?"
					attachTo="${ person.id }" url="/Person.delete.mtw?id=${person.id}"></html:confirm>
			</html:buttonGroup>
		</html:gridColumn>

		<html:toolbar>
			<html:button url="/Person.form.mtw" icon="plus" />
		</html:toolbar>
